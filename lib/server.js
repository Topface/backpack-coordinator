(function(module) {
    ...
    function Server(coordinator) {
        ...
        self.server = http.createServer(function(req, res) {
            var time = new Date();
            var callback = self.responseHandler(res, function(error) {
                self.coordinator.emit("photo.add", !!(error), time);
            });
     ...
     * @param {Function} callback is callback, which is called in any way
     * @returns {Function} Callback to pass in handlers
     */
    Server.prototype.responseHandler = function(res, callback) {
